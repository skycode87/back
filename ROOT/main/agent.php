<?php


define("AGENT_ANALIST",0);
define("AGENT_EXTERN",1);
define("AGENT_ADMIN",9);

   $AGENT['agent_fullname']                       = "dato01";
   $AGENT['agent_document']                       = "dato02";
   $AGENT['agent_phone']                          = "dato03";
   $AGENT['agent_username']                       = "dato04";
   $AGENT['agent_password']                       = "dato05";
   $AGENT['agent_email']                          = "dato06";
   $AGENT['agent_avatar']                         = "dato31";
   $AGENT['agent_token']                          = "token";
   $AGENT['agent_code']                           = "code";
   $AGENT['agent_archived']                       = "archived";
   $AGENT['agent_link']                           = "link";
   $AGENT['agent_created']                        = "created";
   $AGENT['agent_page_initial']                   = "dato07";


?>